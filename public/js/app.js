        ...
        if (hasAccess) {
            ...
            if (queue.id === 'it' && subQueues.length > 0) {
                ...
                mainBtn.addEventListener("click", (e) => {
                    e.stopPropagation();
                    // Select the IT parent queue and show all sub-queue tickets
                    selectedQueue = queue.id;
                    // Set default filter to "not-closed" for Information Technology queue
                    const statusFilter = document.getElementById('status-filter');
                    if (statusFilter) {
                        statusFilter.value = 'not-closed';
                        selectedFilter = 'not-closed';
                    }
                    updateTickets();
                    
                    // Toggle sub-group visibility
...
function updateTickets() {
    ...
    // Set default filter to "not-closed" for IT queue on first selection
    if (selectedQueue === 'it') {
        const statusFilter = document.getElementById('status-filter');
        if (statusFilter) {
            statusFilter.value = 'not-closed';
    ...
    let filtered = tickets.filter(t => {
        ...
        let queueMatch;
        if (selectedQueue === 'it') {
            // Match all IT sub-queues
            queueMatch = t.queue && (t.queue === 'it' || t.queue.startsWith('it-'));
        } else if (selectedQueue && selectedQueue.startsWith("it-")) {
            // Match specific IT sub-queue exactly
        ...
        const filterMatch =
            selectedFilter === "all" ||
            (selectedFilter === "not-closed" && (t.status !== "Closed" && t.status !== "closed")) ||
